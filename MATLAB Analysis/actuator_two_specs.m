function [torque,ang_vel] = actuator_two_specs(L_l2, m_L2, m_M2, L_l3, m_L3, m_M3)
...
%torque from holding link 3 and actuator 3
torque_link3_load = m_L3 * 9.81 * (L_l3/2 + L_l2);
torque_motor3_load = m_M3 * 9.81 * (L_l2);
%also add torque assuming want to apply 3N end effector force
torque_contact = 3 * (L_l3 + L_l2);

%torque from holding link 2
torque_link2_load = m_L2 * 9.81 * (L_l2/2);

%sum loads from links, contact, and actuator...
torque_loads = torque_link3_load + torque_motor3_load + torque_contact + torque_link2_load;
...
%we will do 1) first...
I_motor = calc_I(m_M2, .005); %will assume this as overestimate of mass and length of shaft
torque_motor_accel = I_motor * ang_accel;
%now we will do 2) which is dominant load torque...
%moment of inertia of collective two links and motor3 at middle of links
%(rough estimate)
I_link = calc_I(m_L3 + m_L2 + m_M3, L_l3 + L_l2);
